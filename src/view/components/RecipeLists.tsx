...
export const RecipeLists = ({ onView }: Props) => {
  ...
  const { favorites, hated, removeFromFavorites, removeFromHated } = useRecipeLists();
    ...
    <div className="flex flex-col gap-8 my-8">
      <div>
        ...
        <div className="flex gap-3 flex-wrap">
          {favorites.length === 0 && (
            <span className="text-gray-400 text-sm">Nenhuma favorita ainda.</span>
          )}
      ...
      <div>
        ...
        <div className="flex gap-3 flex-wrap">
          {hated.length === 0 && (
            <span className="text-gray-400 text-sm">Nenhuma detestada ainda.</span>
          )}
          {hated.map(recipe => (
            <ListCard
              key={recipe.title}
